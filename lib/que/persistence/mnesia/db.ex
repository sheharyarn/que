defmodule Que.Persistence.Mnesia.DB do
  ...
  defmodule Jobs do
    ...
    def completed_jobs(name) do
      run_query({:and, {:==, :worker, name}, {:==, :status, :completed}})
    end

    @doc "Find Incomplete Jobs"
    def incomplete_jobs do
      run_query({:or, {:==, :status, :queued}, {:==, :status, :started}})
    end

    @doc "Find Incomplete Jobs for worker"
    def incomplete_jobs(name) do
      run_query(
        {:and, {:==, :worker, name}, {:or, {:==, :status, :queued}, {:==, :status, :started}}}
      )
    end

    @doc "Find Failed Jobs"
    def failed_jobs do
      run_query({:==, :status, :failed})
    end

    @doc "Find Failed Jobs for worker"
    def failed_jobs(name) do
      run_query({:and, {:==, :worker, name}, {:==, :status, :failed}})
    end
